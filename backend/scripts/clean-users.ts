...
async function cleanDb() {
  ...
  try {
    ...
    await prisma.$executeRaw`DELETE FROM public.admin_audit_log WHERE actor_id IN (SELECT unnest(${ids}::uuid[])) OR target_user_id IN (SELECT unnest(${ids}::uuid[]));`;
    console.log('Cleared audit logs.');

    // 2. Staff verifications (reviewed_by does not cascade)
    await prisma.$executeRaw`DELETE FROM auth.staff_verifications WHERE user_id IN (SELECT unnest(${ids}::uuid[])) OR reviewed_by IN (SELECT unnest(${ids}::uuid[]));`;
    console.log('Cleared staff verifications.');

    // 3. User authorization roles (approved_by does not cascade)
    await prisma.$executeRaw`DELETE FROM auth.user_authorization_roles WHERE id IN (SELECT unnest(${ids}::uuid[])) OR approved_by IN (SELECT unnest(${ids}::uuid[]));`;
    ...
    await prisma.$executeRaw`DELETE FROM public.users WHERE id IN (SELECT unnest(${ids}::uuid[]));`;
    console.log('Deleted from public.users.');
    
    await prisma.$executeRaw`DELETE FROM auth.users WHERE id IN (SELECT unnest(${ids}::uuid[]));`;
