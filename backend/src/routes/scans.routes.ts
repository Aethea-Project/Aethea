...
import { createScan, listScans, updateScan, getScanFileHandler, deleteScan } from '../controllers/scans.controller.js';
...
export const createScanRoutes = (authMiddleware: RequestHandler): Router => {
  ...
  router.get('/:id/files/:filename', asyncHandler(getScanFileHandler));
  router.delete('/:id', auth, asyncHandler(deleteScan));
