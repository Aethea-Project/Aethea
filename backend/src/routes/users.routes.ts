...
export const createUserRoutes = (authMiddleware: RequestHandler): Router => {
  ...
  // POST /api/users/profile/upload-avatar-url
  router.post('/profile/upload-avatar-url', protectedAuth, asyncHandler(getAvatarUploadUrl));

  // POST /api/users/password/change-request
  router.post('/password/change-request', authLimiter, passwordChangeAuth, validateBody(requestPasswordChangeSchema), asyncHandler(requestPasswordChange));

  // POST /api/users/password/verify-change
  router.post('/password/verify-change', authLimiter, passwordChangeAuth, validateBody(verifyPasswordChangeSchema), asyncHandler(verifyPasswordChange));
