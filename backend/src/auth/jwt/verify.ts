...
const allowsQueryToken = (req: Request): boolean => {
  const path = req.originalUrl || req.path || '';
  return /\/notifications\/stream(?:\?|$)/.test(path) 
    || /\/jobs\/[^/]+\/progress(?:\?|$)/.test(path)
    || /\/live-queue\/stream(?:\?|$)/.test(path);
};
  ...
  authMiddleware() {
    return async (req: Request, _res: Response, next: NextFunction) => {
      try {
        ...
        // EventSource cannot send auth headers. Keep query tokens limited to SSE routes.
        if (!token && allowsQueryToken(req) && req.query.token && typeof req.query.token === 'string') {
