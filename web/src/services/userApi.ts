...
export const requestAvatarUploadUrl = async (fileName: string): Promise<{ token: string; path: string }> => {
  const response = await authFetch('/users/profile/upload-avatar-url', {
    method: 'POST',
