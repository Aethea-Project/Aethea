...
export function resolveApiBaseUrl(): string {
  // In development, always use relative path so Vite proxy handles it.
  // This automatically fixes ngrok/cloudflared tunnel issues since 
  // requests to tunnel_url/api will route to the Vite dev server proxy.
...
async function prepareAuthHeaders(init?: RequestInit) {
  ...
  // Inject Idempotency-Key for state-changing requests if not already provided
  const method = (init?.method || 'GET').toUpperCase();
  if (['POST', 'PUT', 'PATCH', 'DELETE'].includes(method)) {
    const providedHeaders = (init?.headers as Record<string, string>) || {};
    const hasKey = Object.keys(providedHeaders).some((k) => k.toLowerCase() === 'idempotency-key');
    if (!hasKey && typeof crypto !== 'undefined' && crypto.randomUUID) {
      headers['Idempotency-Key'] = crypto.randomUUID();
    } else if (!hasKey) {
      // Fallback if randomUUID is missing
      headers['Idempotency-Key'] = Math.random().toString(36).substring(2) + Date.now().toString(36);
    }
  }
...
async function handleFetchError(res: Response, path: string) {
  ...
  if (res.status >= 500) {
    ...
    console.error(`[Aethea API Error] ${detailsString}`);
    emitApiError(
      "Server Issue",
      maskedMessage
    );
...
export async function authFetch<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  try {
    ...
    if (err instanceof DOMException && err.name === 'AbortError') {
      emitApiError('Connection Issue', 'The request took too long. Please check your internet connection.');
      throw new Error(`Request to ${path} timed out after ${REQUEST_TIMEOUT_MS / 1000}s`);
    }
    if (err instanceof TypeError) {
      console.error(`[Aethea Network Error] Path: ${path} | Err:`, err);
      emitApiError('Connection Issue', 'Unable to connect to the server. Please check your internet connection.');
...
export async function authFetchStream(path: string, init?: RequestInit): Promise<Response> {
  ...
  try {
    ...
    if (err instanceof TypeError) {
      console.error(`[Aethea Stream Network Error] Path: ${path} | Err:`, err);
      emitApiError('Connection Issue', 'Unable to connect to the server. Please check your internet connection.');
      throw new Error('Unable to connect to the server');
    }
    throw err;
